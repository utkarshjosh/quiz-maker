...
export class GameManager {
  ...
  private handleRevealMessage(data: unknown): void {
    ...
    // Could show correct answer highlight here
  }
  ...
  private handleEndMessage(data: unknown): void {
    ...
    const store = useGameStore.getState();
    store.setScene("leaderboard");
    store.setStatus("finished");

    // Sort players by score for leaderboard
    const sortedPlayers = [...store.players].sort((a, b) => b.score - a.score);
    store.setPlayers(sortedPlayers);
  }
