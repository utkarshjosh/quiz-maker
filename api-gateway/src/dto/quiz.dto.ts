...
export class CreateQuizRoomDto {
  @IsString({ message: 'Quiz ID must be a string' })
  quizId: string;

  @IsOptional()
  @IsInt({ message: 'Max players must be an integer' })
  @Min(1, { message: 'Minimum 1 player required' })
  @Max(100, { message: 'Maximum 100 players allowed' })
  maxPlayers?: number;

  @IsOptional()
  @IsString({ message: 'Room title must be a string' })
  @MinLength(1, { message: 'Room title cannot be empty' })
